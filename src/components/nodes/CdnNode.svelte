<script lang="ts">
  ...
  import type { CdnParams } from '../../types'

  export let id: string
  export let data: CdnParams
...
<div class="arch-node status-{status} px-3 py-2.5 {selected ? 'selected' : ''}" on:click={handleClick}>
  ...
  <div class="flex items-center gap-2 mb-1.5">
    ...
    <span class="text-[10px] uppercase tracking-wider text-slate-500 font-medium">CDN</span>
  </div>

  <div class="flex items-center gap-2 mb-2">
    <span class="text-slate-500"><Globe size={16} /></span>
  ...
  <div class="text-[11px] text-slate-500 space-y-0.5">
    ...
    <div class="flex justify-between gap-3">
      <span>Edge lat.</span>
      <span class="text-slate-700 font-mono">{data.edgeLatencyMs}ms</span>
    </div>
    {#if running && metrics}
