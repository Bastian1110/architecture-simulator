...
<div class="arch-node status-{status} {selected ? 'selected' : ''}" on:click={handleClick}>
  ...
  <div class="flex items-center gap-2 mb-2">
    <span style="color: #3b82f6">
      <svelte:component this={data.subtype === 'mobile' ? Smartphone : Monitor} size={14} />
    </span>
  ...
  <div class="text-[11px] text-slate-400 space-y-0.5">
    <div class="flex justify-between gap-3">
      <span>{data.rps} req/s</span>
      {#if running && metrics}
        <span class="text-slate-600 font-mono">{metrics.rps}/s live</span>
      {/if}
    </div>
  </div>
</div>
